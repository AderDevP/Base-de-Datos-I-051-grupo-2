-- VISTAS (VIEW) - Sistema Apícola - Equipo 02

DROP VIEW IF EXISTS vista_resumen_pedidos_por_usuario;
DROP VIEW IF EXISTS vista_sensores_activos_por_apiario;

-- Vista #1: pedidos por usuario (3 JOIN, WHERE, HAVING, SUM/AVG/MAX/MIN)
CREATE VIEW vista_resumen_pedidos_por_usuario AS
SELECT u.id_usuario, u.nombre AS usuario, r.nombre AS rol,
...
-- Vista #2: sensores por apiario último año (3 JOIN, WHERE, HAVING, COUNT/AVG)
CREATE VIEW vista_sensores_activos_por_apiario AS
SELECT a.id_apiario, a.ubicacion AS apiario, s.tipo,
