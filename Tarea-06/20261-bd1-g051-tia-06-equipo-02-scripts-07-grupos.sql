-- GROUP BY / HAVING - Sistema Apícola - Equipo 02

-- #1 Apicultores por apiario
SELECT a.id_apiario, a.ubicacion AS apiario, COUNT(ua.id_usuario) AS total_apicultores
FROM apiarios a
...
-- #2 Pedidos agrupados por producto
SELECT pr.nombre AS producto,
       COUNT(dp.id_detalle)         AS total_pedidos,
       SUM(dp.cantidad)             AS unidades,
       SUM(dp.cantidad * pr.precio) AS valor_cop
FROM productos pr
INNER JOIN detalle_pedido dp ON dp.id_producto = pr.id_producto
GROUP BY pr.id_producto, pr.nombre
ORDER BY total_pedidos DESC;

-- #3 Sensores por apiario y tipo
SELECT a.ubicacion AS apiario, s.tipo, COUNT(s.id_sensor) AS total
FROM apiarios a
...
-- #4 Pedidos por usuario con total COP (HAVING > 50000)
...
-- #5 Productos pedidos de mayor a menor
SELECT pr.nombre AS producto, pr.precio,
...
SELECT pr.nombre, COUNT(dp.id_detalle) AS total
FROM productos pr
INNER JOIN detalle_pedido dp ON dp.id_producto = pr.id_producto
GROUP BY pr.id_producto, pr.nombre
ORDER BY total ASC LIMIT 1;

-- ¿Usuario con mayor monto COP?
SELECT u.nombre, SUM(dp.cantidad * pr.precio) AS total_cop
FROM usuarios u
INNER JOIN pedidos p         ON p.id_usuario   = u.id_usuario
INNER JOIN detalle_pedido dp ON dp.id_pedido   = p.id_pedido
INNER JOIN productos pr      ON pr.id_producto = dp.id_producto
GROUP BY u.id_usuario, u.nombre
ORDER BY total_cop DESC LIMIT 1;

-- ¿Usuario con más pedidos?
SELECT u.nombre, COUNT(DISTINCT p.id_pedido) AS total_pedidos
FROM usuarios u
INNER JOIN pedidos p         ON p.id_usuario = u.id_usuario
INNER JOIN detalle_pedido dp ON dp.id_pedido = p.id_pedido
GROUP BY u.id_usuario, u.nombre
